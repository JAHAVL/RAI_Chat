# RAI_Chat/backend/rai_api_server.py
...
try:
    ...
    print(f"rai_api_server.py: Added project root to sys.path: {PROJECT_ROOT}") # Use print for startup visibility
except Exception as e:
     print(f"rai_api_server.py: Error adding project root to sys.path: {e}")
...
# --- Import Core Components & Managers (New Paths) ---
try:
    from RAI_Chat.backend.core.database.connection import get_db, test_db_connection # Keep get_db for other endpoints
    from RAI_Chat.backend.core.auth.service import AuthService
    from RAI_Chat.backend.core.auth.models import UserCreateSchema, UserLoginSchema, UserSchema
    from RAI_Chat.backend.managers.user_session_manager import UserSessionManager
    # ConversationManager is now obtained via UserSessionManager
    # from RAI_Chat.backend.managers.conversation_manager import ConversationManager
    # Other managers are also obtained via UserSessionManager
    # from RAI_Chat.backend.managers.chat_file_manager import ChatFileManager
    # from RAI_Chat.backend.managers.memory.contextual_memory import ContextualMemoryManager
    # from RAI_Chat.backend.managers.memory.episodic_memory import EpisodicMemoryManager

    # Import LLM components (assuming path is correct)
    from llm_Engine.llm_api_bridge import get_llm_api
    from RAI_Chat.backend.utils.path_manager import get_user_base_dir # Needed for filesystem listing
...
from RAI_Chat.backend.core.auth.utils import token_required, generate_token
...
@token_required
def chat():
    """Chat endpoint using refactored managers, supporting streaming."""
...
if __name__ == '__main__':
    ...
    try:
        ...
        if is_waitress_available:
            ...
            serve(app, host='::0', port=port, threads=8) # Listen on all interfaces (IPv4 and IPv6)
        else:
