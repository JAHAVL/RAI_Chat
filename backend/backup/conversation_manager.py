...
# Import path manager functions - Keep for LOGS_DIR and ensure_directory_exists for now
from utils.path import LOGS_DIR, ensure_directory_exists # Corrected import path
# Removed other path_manager imports if not directly used here
...
# Built-in Modules (potentially including web search later)
# TODO: Update path if moved
# from modules.calendar_module.calendar_manager import CalendarManager # Removed - Module does not exist

import time
import sys
import traceback

# Import the LLM API with better error handling
import sys
import os
import importlib.util

# Add the correct path to the LLM Engine module
llm_engine_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../llm_Engine'))
sys.path.append(llm_engine_path)

# Log the LLM Engine path
logging.getLogger(__name__).info(f"LLM Engine path: {llm_engine_path}")

# Try to import in a way that shows clear errors
try:
    spec = importlib.util.spec_from_file_location(
        "llm_api_bridge", 
        os.path.join(llm_engine_path, "llm_api_bridge.py")
    )
    if spec and spec.loader:
        llm_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(llm_module)
        get_llm_api = llm_module.get_llm_api
        get_llm_engine = llm_module.get_llm_engine
        logging.getLogger(__name__).info("Successfully imported LLM API bridge module")
    else:
        logging.getLogger(__name__).error(f"Could not load LLM API bridge from {llm_engine_path}")
    ...
    logging.getLogger(__name__).error(f"Error importing LLM API bridge: {e}", exc_info=True)
    # Define fallback functions
    def get_llm_api():
        return None
    def get_llm_engine():
...
class ConversationManager:
    ...
    def get_response(self, db: SQLAlchemySession, user_input: str) -> Generator[Dict[str, Any], None, None]:
        ...
        **currently active session**. Orchestrates prompt building, LLM calls,
        ...
        # Add the last search results to the response data if we have them and if this was a search action
        if action_type == ACTION_SEARCH and last_search_results and not "error" in processed_response_text.lower():
            self.logger.info(f"Adding web search results to final response (truncated first 100 chars): {last_search_results[:100]}...")
            final_response_data = { 
                "llm_response": { 
                    "response_tiers": { 
                        "tier1": "Search results processed", 
                        "tier2": processed_response_text, 
                        "tier3": processed_response_text 
                    }
                },
                "status": "success",
                "web_search_results": last_search_results  # Include the raw search results
            }
        # Handle Fetching & Rerun (if needed)
        elif action_type == ACTION_FETCH and chunk_id_to_fetch:
