...
try:
    ...
    from core.database.models import Message, User, Session
...
class MockTierSystem:
    def __init__(self, token_limit=1000):
        ...
        class MockEpisodicMemory:
            ...
            def archive_chunk(self, content):
                self.archived_chunks.append(content)
                return True
                
            def search(self, query):
                return self.archived_chunks
        ...
        self.mock_db = MockDBSession()
        self.context_builder = ContextBuilder(db_session=self.mock_db)
        self.request_parser = RequestParser(db_session=self.mock_db)
    ...
    def process_assistant_response(self, response, prompt):
        ...
        tier_requests = self.request_parser._find_tier_requests(response)
        episodic_request = self.request_parser._find_episodic_request(response)
        ...
        if episodic_request:
            search_results = self.search_episodic(episodic_request)
            
            # Log the search results
            if search_results:
                logger.info(f"Found {len(search_results)} archived messages about '{episodic_request}'")
        ...
        clean_response = self.request_parser._remove_tier_requests(self.request_parser._remove_episodic_requests(response))
